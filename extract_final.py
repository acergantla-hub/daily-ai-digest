...
# Article 1 - OpenAI (from Wayback Machine)
...
# Article 2 - Washington Post (from Wayback Machine)
with open('article2_wayback2.html', 'r', encoding='utf-8', errors='ignore') as f:
    html = f.read()
desc_m = re.search(r'<meta property="og:description" content="([^"]+)"', html)
desc = desc_m.group(1).strip() if desc_m else 'No description available'
title_m = re.search(r'<meta property="og:title" content="([^"]+)"', html)
title = title_m.group(1).strip() if title_m else 'Unknown title'
results.append({'title': title, 'description': desc, 'url': 'https://www.washingtonpost.com/technology/2026/06/26/openai-says-us-government-will-vet-users-its-latest-ai-model/', 'source': 'washingtonpost.com'})
